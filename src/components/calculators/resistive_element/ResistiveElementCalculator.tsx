...
import WirePresetsModal, { type WirePreset } from "./WirePresetsModal";
...
const CalculatorSection = ({ title, icon, children }: CalculatorSectionProps) => (
  <Card>
...
const Field = ({ label, id, unit, value, onChange, isReadOnly = false, activeField, setActiveField }: FieldProps) => (
  <div
...
interface ResistiveElementValues {
  voltage: string;
  wattage: string;
  current: string;
  resistance: string;
  ...
  length: string;
  surfaceArea: string;
  meanCoilDiameter: string;
  turnSpacing: string;
  turnCircumference: string;
  turns: string;
  coilLength: string;
  wireSurfaceLoad: string;
}

interface ResistiveElementCalculatorProps {
  activeField: string | null;
  setActiveField: (id: string) => void;
}

export default function ResistiveElementCalculator({ activeField, setActiveField }: ResistiveElementCalculatorProps) {
  const [values, setValues] = useState<ResistiveElementValues>({
    // Electrical
    voltage: "120",
    wattage: "1500",
    current: "",
