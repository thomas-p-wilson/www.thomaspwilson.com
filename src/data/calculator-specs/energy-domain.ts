...
export const solarPanels: CalculatorSpec = {
  ...
  sections: [{
    title: "Demand & Panels",
    fields: [
      { id: "dailyDemand", label: "Daily Demand", unit: "Wh" },
      { id: "insolation", label: "Peak Sun Hours / Day", unit: "h" },
      { id: "panelRating", label: "Panel Rating", unit: "W" },
      { id: "panelPrice", label: "Panel Price", unit: "$" },
      { id: "efficiencyModifier", label: "Efficiency Modifier", unit: "fraction (e.g. 0.8)" },
      { id: "demandPerInsolationHour", label: "Demand per Insolation Hour", unit: "W", readOnly: true },
      { id: "realRating", label: "Real Panel Rating", unit: "W", readOnly: true },
...
// Ported from origin/2021-rework/2022's woodgas-conversions calculator.
export const woodgasConversion: CalculatorSpec = {
  slug: "woodgas-conversion",
  title: "Woodgas / Syngas ↔ Gasoline",
  description: "Equivalent syngas volume and biomass required to replace a quantity of gasoline.",
  sections: [{
    title: "Conversion",
    fields: [
      { id: "gasoline", label: "Gasoline", unit: "gal" },
      { id: "woodgasDensity", label: "Syngas Heating Value", unit: "BTU/m³" },
      { id: "gasBTU", label: "Gasoline Energy", unit: "BTU", readOnly: true },
      { id: "gasKJ", label: "Gasoline Energy", unit: "kJ", readOnly: true },
      { id: "gasElectric", label: "Gasoline Energy", unit: "kWh", readOnly: true },
      { id: "woodgas", label: "Equivalent Syngas Volume", unit: "m³", readOnly: true },
      { id: "biomass", label: "Equivalent Biomass (15% moisture)", unit: "kg", readOnly: true },
    ],
  }],
  defaults: { gasoline: "1", woodgasDensity: "7062.93" },
  calculate: (values) => {
    const gasoline = num(values, "gasoline");
    const woodgasDensity = num(values, "woodgasDensity");
    if (isNaN(gasoline) || isNaN(woodgasDensity) || woodgasDensity === 0) return values;

    const gasBTU = gasoline * 19000;
    const gasKJ = gasBTU * 1.05506;
    const gasElectric = gasBTU * 0.00029307107017;
    const woodgas = gasBTU / woodgasDensity;
    const biomass = woodgas / 2.185;

    return {
      ...values,
      gasBTU: gasBTU.toFixed(0),
      gasKJ: gasKJ.toFixed(0),
      gasElectric: gasElectric.toFixed(2),
      woodgas: woodgas.toFixed(2),
      biomass: biomass.toFixed(2),
    };
  },
  notes: [
    "Gasoline assumed at 19,000 BTU/lb, specific gravity 0.71. 1 kg biomass at 15% moisture ≈ 2.185 m³ syngas.",
    "Typical syngas heating values range 3,531–10,594 BTU/m³ depending on gasifier and feedstock.",
  ],
};
...
export const chemicalStorage: CalculatorSpec = {
  ...
  sections: [{
    ...
    fields: [
      ...
      { id: "capacity", label: "Usable Capacity", unit: "kWh" },
      { id: "cellCapacity", label: "Cell Capacity", unit: "Wh" },
      { id: "cellCost", label: "Cell Cost", unit: "$" },
      { id: "taxes", label: "Taxes", unit: "fraction (e.g. 0.13)" },
      { id: "nameplateCapacity", label: "Nameplate Capacity", unit: "kWh", readOnly: true },
