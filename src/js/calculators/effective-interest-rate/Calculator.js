import React, { useContext } from 'react';
import SimpleCalculator from '../../components/redone/SimpleCalculator';
import calculator from './definition';
import { DataContext } from '../helpers';
import EffectiveRateListing from './pages/EffectiveRateListing';
import PeriodicRateTable from './pages/PeriodicRateTable';

export default () => {
	const { data = {} } = useContext(DataContext);

	return (
		<div className="calculator calc-circle-area">
			<div className="calc-column">
				<SimpleCalculator calculator={ calculator } />
			</div>
			<div className="desc-column">
				<EffectiveRateListing data={ data } />
				<PeriodicRateTable data={ data } />
			</div>
		</div>
	);
}
