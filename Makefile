...
# Node
lint-css:
	NODE_ENV=test docker-compose run web yarn sass-lint -c .sass-lint.yml -v

lint-js:
	NODE_ENV=test docker-compose run web yarn eslint .

lint:
	make lint-css && make lint-js

test:
	NODE_ENV=test docker-compose run web yarn mocha --compilers js:babel-core/register -r chai/register-expect -R spec ./src/**/*.tests.js

analyze:
	NODE_ENV=production WEBPACK_ANALYZER=1 docker-compose run web yarn webpack

build:
	NODE_ENV=production docker-compose run web yarn webpack
